...
const arr_columns__env_sensor_types = process.env.ENV_SENSOR_TYPES.split(",");
const arr_columns__sensor_types = [
  ...arr_columns__common_sensor_types,
  ...arr_columns__env_sensor_types,
];
const arr_column_params = arr_columns__sensor_types.map((el, i) => {
  return `VAL${i}`;
});
...
const TUNNEL_COLUMNS = arr_columns__sensor_types.join();
